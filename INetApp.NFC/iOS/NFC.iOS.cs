...
namespace INetApp.NFC
{
    ...
    public class NFCImplementation : NFCTagReaderSessionDelegate, INFC
    {
        ...
        public void StartListening()
        {
            ...
            NfcSession = new NFCTagReaderSession(NFCPollingOption.Iso14443 | NFCPollingOption.Iso15693 | NFCPollingOption.Iso18092, this, DispatchQueue.CurrentQueue)
            {
        ...
        {
            WriteOrClearMessage(_tag, tagInfo, false, makeReadOnly);
        }
        ...
        {
            WriteOrClearMessage(_tag, tagInfo, true);
        }
        ...
        public override void DidDetectTags(NFCTagReaderSession session, INFCTag[] tags)
        {
            ...
            session.ConnectTo(_tag, (error) =>
            {
                ...
                INFCNdefTag ndefTag = GetNdefTag(_tag);
                ...
                ndefTag.QueryNdefStatus((status, capacity, error) =>
                {
                    ...
                    bool isNdefSupported = status != NFCNdefStatus.NotSupported;

                    byte[] identifier = GetTagIdentifier(ndefTag);
                    TagInfo nTag = new TagInfo(identifier, isNdefSupported)
                    {
        ...
        public override void DidInvalidate(NFCTagReaderSession session, NSError error)
        {
            ...
            {
                UIAlertController alertController = UIAlertController.Create(Configuration.Messages.NFCSessionInvalidated, error.LocalizedDescription.ToLower().Equals(SessionTimeoutMessage) ? Configuration.Messages.NFCSessionTimeout : error.LocalizedDescription, UIAlertControllerStyle.Alert);
        ...
        internal void WriteOrClearMessage(INFCTag tag, ITagInfo tagInfo, bool clearMessage = false, bool makeReadOnly = false)
        {
            ...
            INFCNdefTag ndefTag = GetNdefTag(tag);
            if (ndefTag == null)
            {
        ...
        private byte[] GetTagIdentifier(INFCNdefTag tag)
        {
        ...
        private NFCNdefPayload GetiOSPayload(NFCNdefRecord record)
        {
            if (record == null)
            {
                return null;
            ...
            switch (record.TypeFormat)
            {
                case NFCNdefTypeFormat.WellKnown:
                    string lang = record.LanguageCode;
                    if (string.IsNullOrWhiteSpace(lang))
                    {
                        lang = Configuration.DefaultLanguageCode;
                    }

                    byte[] langData = Encoding.ASCII.GetBytes(lang.Substring(0, 2));
                    byte[] payloadData = new byte[] { 0x02 }.Concat(langData).Concat(record.Payload).ToArray();
        ...
        private NFCNdefPayload GetEmptyPayload()
        {
        ...
        private NFCNdefMessage GetEmptyNdefMessage()
        {
            NFCNdefPayload[] records = new NFCNdefPayload[1];
        ...
        private void MakeTagReadOnly(NFCTagReaderSession session, INFCTag tag, INFCNdefTag ndefTag)
        {
    ...
    public class NFCImplementation_Before_iOS13 : NFCNdefReaderSessionDelegate, INFC
    {
        ...
        public void StartPublishing(bool clearMessage = false)
        {
            throw new NotSupportedException(Configuration.Messages.NFCWritingNotSupported);
        }
        ...
        public override void DidDetect(NFCNdefReaderSession session, NFCNdefMessage[] messages)
        {
            ...
            {
                NFCNdefMessage first = messages[0];
    ...
    internal static class NFCNdefMessageExtensions
    {
        ...
        {
            NFCNdefPayload[] records = message?.Records;
            ...
            {
                NFCNdefPayload record = records[i];
                NFCTypeNameFormat typeNameFormat = record?.TypeNameFormat ?? NFCTypeNameFormat.Empty;
                NSData payload = record?.Payload;
                NSData id = record?.Identifier;
                NSData type = record?.Type;

                byte flags = (byte)typeNameFormat;
    ...
    internal static class NFCNdefPayloadExtensions
    {
        ...
        public static Uri ToUri(this NFCNdefPayload payload)
        {
            switch (payload.TypeNameFormat)
            {
                case NFCTypeNameFormat.NFCWellKnown:
                    ...
                    {
                        Uri uri = payload.Payload.ParseWktUri();
                        return uri;
                    }
                    break;
                ...
                case NFCTypeNameFormat.Media:
                    ...
                    if (Uri.TryCreate(content, UriKind.RelativeOrAbsolute, out Uri result))
                    {
                        return result;
                    }
        ...
        private static Uri ParseWktUri(this NSData data)
        {
            ...
            string prefix = _uri_Prefixes_Map[prefixIndex];
            string suffix = Encoding.UTF8.GetString(CopyOfRange(payload, 1, payload.Length));

            if (Uri.TryCreate(prefix + suffix, UriKind.Absolute, out Uri result))
            {
                return result;
            }
